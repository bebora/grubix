...
export function LightRenderer(lightState, gl, program) {
    this.initUniforms = function() {
        ...
        for (let i = 0; i < 10; i++) {
            directUniforms.push({
                valid: gl.getUniformLocation(program, `${directUniformPrefix}[${i}].valid`),
                color: gl.getUniformLocation(program, `${directUniformPrefix}[${i}].color`),
                direction: gl.getUniformLocation(program, `${directUniformPrefix}[${i}].direction`),
            })
        }
        ...
        for (let i = 0; i < 10; i++) {
            pointUniforms.push({
                valid: gl.getUniformLocation(program, `${pointUniformPrefix}[${i}].valid`),
                position: gl.getUniformLocation(program, `${pointUniformPrefix}[${i}].position`),
                color: gl.getUniformLocation(program, `${pointUniformPrefix}[${i}].color`),
                decay: gl.getUniformLocation(program, `${pointUniformPrefix}[${i}].decay`),
                distance: gl.getUniformLocation(program, `${pointUniformPrefix}[${i}].distance`),
            })
        }
        ...
        for (let i = 0; i < 10; i++) {
            spotUniforms.push({
                valid: gl.getUniformLocation(program, `${spotUniformPrefix}[${i}].valid`),
                direction: gl.getUniformLocation(program, `${spotUniformPrefix}[${i}].direction`),
                position: gl.getUniformLocation(program, `${spotUniformPrefix}[${i}].position`),
                color: gl.getUniformLocation(program, `${spotUniformPrefix}[${i}].color`),
                decay: gl.getUniformLocation(program, `${spotUniformPrefix}[${i}].decay`),
                distance: gl.getUniformLocation(program, `${spotUniformPrefix}[${i}].distance`),
                coneOut: gl.getUniformLocation(program, `${spotUniformPrefix}[${i}].coneOut`),
                coneIn: gl.getUniformLocation(program, `${spotUniformPrefix}[${i}].coneIn`),
            })
        }
    ...
    this.injectUniform = function(viewMatrix) {
        ...
        for (let i = 0; i < 10; i++) {
            if (i < directLights.length) {
                ...
                let directionalLightTransformed = mathUtils.multiplyMatrix3Vector3(
                    mathUtils.sub3x3from4x4(mathUtils.invertMatrix(
                        mathUtils.transposeMatrix(viewMatrix))),
                    parameters.direction
                );
                gl.uniform1i(uniforms.direct[i].valid, 1);
                gl.uniform3f(uniforms.direct[i].direction, ...directionalLightTransformed);
        ...
        for (let i = 0; i < 10; i++) {
            if (i < pointLights.length) {
                ...
                let positionLightTransformed = mathUtils.multiplyMatrices(
                    viewMatrix,
                    [parameters.position[0], parameters.position[1], parameters.position[2], 1]
                );
                gl.uniform1i(uniforms.point[i].valid, 1);
                gl.uniform3f(uniforms.point[i].position, ...(positionLightTransformed.slice(0,3)));
        ...
        for (let i = 0; i < 10; i++) {
            if (i < spotLights.length) {
                ...
                gl.uniform3f(uniforms.spot[i].direction, ...directionalLightTransformed);
                gl.uniform3f(uniforms.spot[i].position, ...(positionLightTransformed.slice(0,3)));
